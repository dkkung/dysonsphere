...
def _render_provenance(prov: dict) -> str:
    ...
    sentence = (
        ...
        f"vl-convert {deps['vl_convert']}, dysonsphere {deps['dysonsphere']}, "
...
def _build_provenance(*, export_id: str, timestamp: str, checksum: str, data_checksum: list[str]) -> dict:
    ...
    interpreter, spec generator, RENDERER, tool, and its runtime dependencies; ``os`` + ``vl_convert``
    together pin *rendering* - vl-convert resolves fonts partly from the system and bundles its own
    Vega/Vega-Lite build, so text metrics can differ across platforms and renderer versions) — and
    finally the machine
