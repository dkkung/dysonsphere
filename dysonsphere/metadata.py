...
def _build_provenance(*, export_id: str, timestamp: str, checksum: str, data_checksum: list[str]) -> dict:
    ...
    the platform + versioned runtime stack (``os`` from ``platform.platform()`` first, then
    ``python``/``altair``/``dysonsphere``/``numpy``/``scipy``/``polars``: the OS, interpreter, tool,
    and its four declared runtime dependencies; ``os`` matters because vl-convert resolves fonts
    partly from the system, so text metrics can differ across platforms) — and finally the machine
