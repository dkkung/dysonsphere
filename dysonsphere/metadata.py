...
def _render_provenance(prov: dict) -> str:
    ...
    sentence = (
        ...
        f"using Python {deps['python']}, Altair {deps['altair']}, dysonsphere {deps['dysonsphere']}, "
...
def _build_provenance(*, export_id: str, timestamp: str, checksum: str, data_checksum: list[str]) -> dict:
    ...
    (``user``/``script``/``timestamp``), and finally ``environment`` — the versioned runtime
    stack (``python``/``altair``/``dysonsphere``/``numpy``/``scipy``/``polars``: the interpreter,
    the tool, and its four declared runtime dependencies), grouped so the versions that determine
    whether a reported statistic reproduces sit in one place.
    """
    try:
