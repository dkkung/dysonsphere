...
def _render_provenance(prov: dict) -> str:
    ...
    bits = [deps["os"], f"Python {deps['python']}", f"Altair {deps['altair']}"]
    if deps.get("vl_convert"):  # renderer; absent for json-only exports
        bits.append(f"vl-convert {deps['vl_convert']}")
    bits += [
        f"dysonsphere {deps['dysonsphere']}",
        f"NumPy {deps['numpy']}",
        f"SciPy {deps['scipy']}",
        f"Polars {deps['polars']}",
    ]
    sentence = f"Generated by {prov['user']} with {prov['script']} on {prov['timestamp']} using {', '.join(bits)}."
...
def _build_provenance(*, export_id: str, timestamp: str, checksum: str, data_checksum: list[str]) -> dict:
    ...
    ``python``/``altair``/``vl_convert``/``dysonsphere``/``numpy``/``scipy``/``polars``: the OS,
    interpreter, spec generator, RENDERER, tool, and its four declared runtime dependencies; ``os``
    and ``vl_convert`` together pin *rendering* - vl-convert resolves fonts partly from the system and
    bundles its own Vega/Vega-Lite build, so text metrics can differ across platforms and renderer
    versions. ``vl_convert`` is ``None`` when not installed - a json-only export needs no renderer) —
    ...
    try:
        ...
        vl_convert = None  # not installed (json-only export needs no renderer)
    return {
        "user": user,
        "script": script,
        "timestamp": timestamp,
