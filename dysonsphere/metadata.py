...
def _render_provenance(prov: dict) -> str:
    ...
    sentence = (
        ...
        f"dysonsphere {deps['dysonsphere']}, NumPy {deps['numpy']}, SciPy {deps['scipy']}, "
        f"Polars {deps['polars']}."
    )
...
def _build_provenance(*, export_id: str, timestamp: str, checksum: str, data_checksum: list[str]) -> dict:
    ...
    ``checksum`` is per-variant (the SHA-256 of that variant's spec).  ``dataChecksum`` leads
    (the data identity, one entry per user frame, order-independent), then ``vegaliteChecksum``
    (the content identity), then ``exportIdentifier`` (the run), the who/when context
    (``user``/``script``/``timestamp``), and finally ``environment`` — the platform + versioned
    runtime stack (``os`` from ``platform.platform()`` first, then
    ``python``/``altair``/``dysonsphere``/``numpy``/``scipy``/``polars``: the OS, interpreter, tool,
    and its four declared runtime dependencies), grouped so everything that determines whether a
    figure/statistic reproduces sits in one place (``os`` matters because vl-convert resolves fonts
    partly from the system, so text metrics can differ across platforms).
    """
    try:
...
def _user_datasets(spec) -> dict[str, list]:
...
def _build_block(
    ...
    *,
    ...
    provenance = _build_provenance(
        export_id=export_id, timestamp=timestamp, checksum=checksum, data_checksum=data_checksum
    )
