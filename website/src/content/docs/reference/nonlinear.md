...
- **`minorTickSize`** (`float | None`) - Length of minor ticks in pixels. Defaults to half the active theme's ``tickSize`` (``tickSize / 2``; typically ``1.5`` when the default ``tickSize=3.0`` is in effect).
