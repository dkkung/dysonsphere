...
- **`strokeDash`** (`bool | list[int | float] | None`) - ``None`` (default) inherits the theme's ``ruleStrokeDash`` setting. ``False`` forces a solid line. ``True`` uses the theme's ``strokeDash`` pattern. A list (e.g. ``[4, 2]``) uses that pattern directly.
...
- **`strokeDash`** (`list[float] | bool | None`) - Dash pattern for the rect border. ``None`` (default) → solid. ``True`` → inherit ``strokeDash`` from the active theme. A list (e.g. ``[4, 2]``) → use that pattern directly.
