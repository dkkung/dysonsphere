...
- **`labelFontSize`** (`float`) - Figure-label styling. Weight is numeric (700, bold, by default). ``labelColor`` defaults to the theme's title ink, which follows ``dark`` at render, so a ``save()`` across both backgrounds gets the right color without a callable. ``labelOffset`` offsets the label from the corner - one number for both axes, or ``(x, y)``. It defaults to ``(-5, 0)``, holding the label off the chart the way ``axisOffset`` detaches the axes. The label already sits at the figure's leftmost point, so a negative x cannot move it further left - it widens the canvas and indents the chart instead, which reads the same and costs those pixels of width.
