...
# the australis LUT and served as an image; the colorbar is a matching australis color scale.
...
image = (
    ...
    .mark_image(url="/gallery/condensate_australis.png", aspect=False)
    .encode(
...
# an invisible layer whose continuous color scale draws the australis colorbar (a.u.)
colorbar = (
    ...
    .encode(
        x="x:Q",
        y="y:Q",
        color=alt.Color("intensity:Q", title="intensity (a.u.)", scale=alt.Scale(range=colors["australis"])),
    )
)
