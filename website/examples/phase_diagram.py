...
lines = (
    ...
    .encode(
        ...
        y=alt.Y(
            "P:Q",
            title="Pressure (bar)",
            scale=alt.Scale(type="log", domain=[1, 300], nice=False),
...
pts = (
    alt.Chart(pl.DataFrame({"T": [Tt, Tc], "P": [Pt, Pc]}))
    .mark_point(size=28, filled=True, opacity=1)
    .encode(x="T:Q", y="P:Q")
)
...
visual = (
    lines
    + pts
    + ds.text("Solid", x=200.0, y=40.0)
    + ds.text("Liquid", x=248.0, y=120.0)
    + ds.text("Gas", x=250.0, y=2.0)
    + ds.text("Supercritical", x=312.0, y=140.0, align="right")
    + ds.text("Triple", x=Tt, y=Pt, offsetX=-4, align="right")
    + ds.text("Critical", x=Tc, y=34.0, offsetX=4, align="right")
)
