...
- **The `presentation` built-in style preset is removed.** `theme(style="presentation")` now raises the standard unknown-style `ValueError` unless you define a `[presentation]` section in your own `dysonsphere.toml` (which keeps working exactly as before - config-file styles are unaffected). The `create_config()` template no longer scaffolds it. `notebook` remains the only shipped preset.
...
- **`theme(transparentBackground=)` renamed to `theme(transparent=)`.** Shorter, and it now pairs with the `save(transparent=)` parameter - the same question answered at the theme level (the chart's logical background: notebook display, JSON, HTML) and per export. The old name still works everywhere it did before (keyword argument, `dysonsphere.toml` keys, and the baked theme in previously exported JSONs loaded via `load()`) but emits a `DeprecationWarning`; it will be removed in v4.0 (kept through v3.x so `load()` of previously exported files keeps working).
