...
- **`add_comparisons()` per-pair `bracketStyle`** - `bracketStyle` now also accepts a `dict` mapping a pair to its style, e.g. `{("A","B"): "line", ("A","C"): "bracket"}`, for mixed bracket styles in one call (keys match either pair order; unlisted pairs default to `"bracket"`). Plain `"line"`/`"bracket"` strings still style every bracket uniformly.
