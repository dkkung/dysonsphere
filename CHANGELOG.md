...
- **`save()` provenance records the rendering toolchain (OS + vl-convert).** The embedded provenance block's `environment` group gains `os` (from `platform.platform()`, e.g. `macOS-26.5.2-arm64-arm-64bit`) as its first entry, and `vl_convert` (the renderer's version) after `altair`. `environment` already pinned the interpreter and the numerical stack (NumPy/SciPy/Polars); OS and vl-convert together pin *rendering* - vl-convert resolves fonts partly from the system and bundles its own Vega/Vega-Lite build, so text metrics (superscript alignment, `add_labels` label placement) can differ across platforms and renderer versions. `vl_convert` is `null` when not installed (a json-only export needs no renderer). OS is recorded coarsely (no hostname or other identifying detail).
