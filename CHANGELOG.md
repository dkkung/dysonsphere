...
- **`save()` provenance records the rendering toolchain (OS + vl-convert).** The provenance block's `environment` group gains `os` (from `platform.platform()`, e.g. `macOS-26.5.2-arm64-arm-64bit`) as its first entry and `vl_convert` (the renderer's version) after `altair`. `environment` already pinned the interpreter and the numerical stack (NumPy/SciPy/Polars); OS and vl-convert together pin *rendering* - vl-convert resolves fonts partly from the system and bundles its own Vega/Vega-Lite build, so text metrics (superscript alignment, `add_labels` label placement) can differ across platforms and renderer versions. OS recorded coarsely (no hostname or other identifying detail).

### Changes

- **`vl-convert-python` is now a project dependency** (was dev-group-only). `save()` to PNG/SVG/HTML and `show()`'s accurate preview require it, so a fresh `pip install dysonsphere` could `ImportError` on a core feature; it's now always installed. dysonsphere is export-first, so it treats the renderer as a hard requirement (unlike altair, which keeps static rendering optional because it's interactive-first).
- **Provenance fields are ordered readable-first.** The `provenance` block now reads `user → script → timestamp → environment → vegaliteChecksum → exportIdentifier → dataChecksum`: the human-readable context leads (matching the rendered "Generated by …" prose), and the machine-identity checksums trail, with the one unbounded field - `dataChecksum`, a list that grows with multi-frame charts - last, so the readable top of the block stays stable. This is a key-order change only; the block is key-addressed, so `read()` and any consumer are unaffected.
