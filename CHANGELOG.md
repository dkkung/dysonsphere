...
- **`save()` `embedReport=`** - the human-readable statistics report table is now embedded in every output by default (`True`), so you can read it straight out of a saved file: as a `report` member of `usermeta.dysonsphere` in the JSON, and as a dedicated readable channel (real newlines) in the SVG (`<metadata id="dysonsphere-report">`) and PNG (`iTXt dysonsphere-report`). It never touches `description`. Set `embedReport=False` to keep only the structured block.
