...
- **`add_comparisons(xOffsetCol=...)` now raises a clear error when `categories`/`xOffsetSort` don't cover the data.** In grouped mode these must match your chart's `x`/`xOffset` sort, or the shared scale silently reorders the bars. The function can't see the chart to check the *order*, but an explicit list that omits a value present in the data (a typo or omission) is a guaranteed mismatch - it now raises naming the missing values instead of rendering a mysteriously reordered chart.
