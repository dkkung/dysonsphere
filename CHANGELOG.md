...
- **`add_rule(data=)` facet-safe mode.** Reference lines couldn't be faceted: by default `add_rule` carries its own small dataset, and Altair refuses to facet a layered chart whose layers don't all share one data variable (and the `facet(data=)` workaround silently fails to partition). Passing `data=` (the same DataFrame as the base chart) switches to datum mode - the rule shares that frame and is positioned by a constant `alt.datum`, so `(base + add_rule(..., data=df)).facet(...)` works and the line repeats in every panel. Accepts polars or pandas; the default (no `data=`) behavior is unchanged.
